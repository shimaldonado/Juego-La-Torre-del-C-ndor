...
export class GameScene extends Phaser.Scene {
  ...
  createBossArena() {
    ...
    // Sin pinchos en la arena del boss
  ...
  createLadder(x, fromY, toY) {
    ...
    this.ladders.push({ x, rect: new Phaser.Geom.Rectangle(x - 38, y - height / 2, 76, height), visual: g });
  }
  ...
  createPlayer(x, y, character, label, tint, state) {
    const idleKey = character.id === 'killa' ? 'killa_idle' : character.id === 'rumi' ? 'rumi_idle' : 'heroe_idle';
    const sprite = this.physics.add.sprite(x, y, idleKey)
      .setScale(0.72).setDepth(12);
    ...
    const prefix = character.id === 'killa' ? 'killa' : character.id === 'rumi' ? 'rumi' : 'hero';
    const idleTexture = character.id === 'killa' ? 'killa_idle' : character.id === 'rumi' ? 'rumi_idle' : 'heroe_idle';
    return {
      sprite,
      name,
      character,
  ...
  createUI() {
    ...
    this.powerCooldownText = this.add.text(640, 690, '', {
      fontSize: '20px', color: '#fbbf24', fontStyle: 'bold', stroke: '#000', strokeThickness: 3,
  ...
  updatePlayer(player, controls, time) {
    ...
    // En escaleras: se desactiva temporalmente la colisión vertical para que
    // el personaje sí pueda atravesar el borde de la plataforma y subir/bajar.
    if (onLadder && up) {
      // Subir escalera: desactivar colisión arriba y abajo para atravesar plataforma
      sprite.body.allowGravity = false;
      sprite.body.checkCollision.up = false;
      sprite.body.checkCollision.down = false;
      sprite.setVelocityY(-185);
      sprite.x = Phaser.Math.Linear(sprite.x, currentLadder.x, 0.25);
    } else if (onLadder && down && !sprite.body.blocked.down) {
      // Bajar escalera: solo desactivar colisión arriba (no abajo, para no caer de largo)
      sprite.body.allowGravity = false;
      sprite.body.checkCollision.up = false;
      ...
      // En escalera sin moverse: pausar
      sprite.body.allowGravity = false;
      sprite.body.checkCollision.up = false;
      sprite.body.checkCollision.down = false;
      sprite.setVelocityY(0);
    } else {
      // Fuera de escalera: física normal
    ...
    // Poder especial (Q para P1, SHIFT para P2) — solo Killa y Rumi
    if (controls.power && Phaser.Input.Keyboard.JustDown(controls.power)) {
      this.activatePower(player, time);
    }
    ...
    const pfx = player.prefix;
    if (time < player.poderActiveUntil) sprite.play(`${pfx}_power`, true);
    else if (time < player.attackingUntil) sprite.play(`${pfx}_hit`, true);
    else if (!sprite.body.blocked.down && sprite.body.velocity.y < -30) sprite.play(`${pfx}_jump`, true);
    else if (!sprite.body.blocked.down && sprite.body.velocity.y > 60) sprite.play(`${pfx}_fall`, true);
    else if (Math.abs(sprite.body.velocity.x) > 10) sprite.play(`${pfx}_run`, true);
    else { sprite.anims.stop(); sprite.setTexture(player.idleTexture); }
  }
  ...
  activatePower(player, time) {
    const { character, prefix } = player;

    // Auki: lanza remolino dorado hacia adelante (ataque_dorado)
    // Killa: mascota — lanza remolino dorado en área
    // Rumi: golpe de piedra — lanza remolino dorado hacia adelante más potente

    if (time < player.poderCooldownUntil) {
      const secsLeft = Math.ceil((player.poderCooldownUntil - time) / 1000);
      this.showPowerLabel(player.sprite.x, player.sprite.y, `⏳ Poder en ${secsLeft}s`, 0xfbbf24);
      return;
    }

    const sx = player.sprite.x;
    const sy = player.sprite.y - 20;
    const dir = player.sprite.flipX ? -1 : 1;

    if (prefix === 'hero') {
      // ── AUKI: Remolino dorado — proyectil recto ──
      player.poderCooldownUntil = time + 6000;
      player.poderActiveUntil   = time + 500;
      this.launchEffect(sx, sy, dir, 'ataque_dorado', 480, 1.1, (enemy) => {
        this.score += 120;
      });
      this.showPowerLabel(sx, sy, '🌀 ¡Remolino Dorado!', 0xffcc5c);
      AudioManager.play(this, 'sfx_hit', { volume: 0.8 });

    } else if (prefix === 'killa') {
      // ── KILLA: Zorro Lunar — 3 remolinos en abanico ──
      player.poderCooldownUntil = time + 8000;
      player.poderActiveUntil   = time + 700;
      // Lanza 3 remolinos: recto, arriba y abajo
      const angles = [0, -0.35, 0.35];
      angles.forEach((angle, i) => {
        this.time.delayedCall(i * 80, () => {
          this.launchEffectAngle(sx, sy, dir, angle, 'ataque_dorado', 420, 0.9, (enemy) => {
            this.score += 150;
          });
        });
      });
      this.cameras.main.shake(160, 0.006);
      this.showPowerLabel(sx, sy, '🦊 ¡Zorro Lunar! ×3', 0x67e8f9);
      AudioManager.play(this, 'sfx_hit', { volume: 0.85 });

    } else if (prefix === 'rumi') {
      // ── RUMI: Puño de Piedra — remolino gigante + escudo ──
      player.poderCooldownUntil = time + 9000;
      player.poderActiveUntil   = time + 800;
      this.launchEffect(sx, sy, dir, 'ataque_dorado', 360, 1.6, (enemy) => {
        this.score += 180;
        // También hace daño al boss
        if (enemy === this.boss) this.hitBoss(2);
      });
      // Escudo dorado 2.5s
      player.invulnerableUntil = time + 2500;
      player.sprite.setTint(0xfbbf24);
      this.time.delayedCall(2500, () => { if (player.sprite?.active) player.sprite.clearTint(); });
      this.cameras.main.shake(220, 0.01);
      this.showPowerLabel(sx, sy, '🪨 ¡Puño de Piedra! + escudo', 0xd97706);
      AudioManager.play(this, 'sfx_hit', { volume: 0.9 });
    }
  }

  // Lanza efecto visual recto que elimina enemigos al tocar
  launchEffect(x, y, dir, textureKey, speed, scale, onHit) {
    if (!this.textures.exists(textureKey)) return;
    const proj = this.add.image(x, y, textureKey)
      .setScale(scale).setDepth(90).setFlipX(dir < 0);
    let traveled = 0;
    const maxDist = 700;
      ...
      callback: () => {
        ...
        this.getGroupChildren(this.enemies).forEach((enemy) => {
          ...
          if (dist < 70) {
            ...
            onHit(enemy);
            this.tweens.add({ targets: enemy, alpha: 0, scaleX: 1.3, scaleY: 1.3, duration: 250,
              onComplete: () => enemy.active && enemy.disableBody(true, true) });
          }
        });
        if (traveled > maxDist) { step.remove(); proj.destroy(); }
      },
    });
    // Fade out al final
    this.time.delayedCall(maxDist / speed * 1000 - 200, () => {
      if (proj.active) this.tweens.add({ targets: proj, alpha: 0, duration: 200, onComplete: () => proj.destroy() });
      step.remove();
    });
  }

  // Versión con ángulo para el abanico de Killa
  launchEffectAngle(x, y, dir, angleRad, textureKey, speed, scale, onHit) {
    if (!this.textures.exists(textureKey)) return;
    const proj = this.add.image(x, y, textureKey)
      .setScale(scale).setDepth(90).setFlipX(dir < 0);
    const vx = dir * speed * Math.cos(angleRad);
    ...
    this.time.delayedCall(maxDist / speed * 1000 - 150, () => {
      if (proj.active) this.tweens.add({ targets: proj, alpha: 0, duration: 150, onComplete: () => proj.destroy() });
      step.remove();
    });
  }

  showPowerLabel(worldX, worldY, text, color) {
    // Convertir coords de mundo a pantalla para que se vea siempre
    const cam = this.cameras.main;
    const sx = worldX - cam.scrollX;
    const sy = worldY - cam.scrollY - 60;
    const hex = `#${color.toString(16).padStart(6,'0')}`;
    const label = this.add.text(sx, sy, text, {
      fontSize: '24px', color: hex,
      fontStyle: 'bold', stroke: '#000', strokeThickness: 5,
    }).setOrigin(0.5).setDepth(200).setScrollFactor(0);
    this.tweens.add({
      targets: label, y: sy - 80, alpha: 0, duration: 1400,
  ...
  tryAttackEnemy(player) {
    ...
    this.getGroupChildren(this.enemies).forEach((enemy) => {
      ...
      if (Phaser.Geom.Circle.Contains(range, enemy.x, enemy.y)) this.damageEnemy(enemy, 1, 130, player);
    });
  }
  ...
  showAttackEffect(player) {
    ...
    const slash = this.add.image(player.sprite.x + direction * 36, player.sprite.y - 8, 'ataque_dorado')
      .setDepth(16)
      .setScale(0.34)
      .setAlpha(0.95)
  ...
  handlePlayerDown(player) {
    ...
    player.sprite.play(`${player.prefix}_death`, true);
