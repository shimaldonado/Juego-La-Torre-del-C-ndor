...
export class GameScene extends Phaser.Scene {
  ...
  updatePlayer(player, controls, time) {
    ...
    // En escaleras: el personaje puede subir y bajar,
    // pero no debe pasarse por debajo de la plataforma.
    if (onLadder && (up || down)) {
      sprite.body.allowGravity = false;
      ...
      // Si currentLadder tiene topY/bottomY los usa.
      // Si no, usa el rectángulo de la escalera.
      ...
      // Bajar
        ...
        if (sprite.y >= bottomLimit) {
          ...
          // Aquí vuelve a activar la gravedad y colisión
          // para que el personaje quede sobre la plataforma.
          sprite.body.allowGravity = true;
          sprite.body.checkCollision.up = true;
          sprite.body.checkCollision.down = true;
        } else {
    ...
    const attackPressed = Phaser.Input.Keyboard.JustDown(controls.attack) || controls.touchAttack;
    ...
    } else if (!sprite.body.blocked.down && sprite.body.velocity.y < -30) {
      sprite.play(anims.jump, true);
    } else if (!sprite.body.blocked.down && sprite.body.velocity.y > 60) {
