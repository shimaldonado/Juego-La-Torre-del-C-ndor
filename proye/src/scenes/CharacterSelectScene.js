...
export class CharacterSelectScene extends Phaser.Scene {
  ...
  init(data = {}) {
    this.playerCount = data.playerCount || data.mode || 1;
    this.selected = data.selected || data.characterId || 'auki';
  }

  create() {
    const { width, height } = this.scale;

    // Fondo claro de la torre, sin el fondo verde/negro del menú anterior.
    addScreenBackground(this, 'fondo_nivel1');

    this.add.rectangle(width / 2, height / 2, width, height, COLORS.navy, 0.28)
      .setDepth(0);
  ...
  createCharacterCard(char, x, y) {
    ...
    // Se mantiene el movimiento dinámico de los personajes.
