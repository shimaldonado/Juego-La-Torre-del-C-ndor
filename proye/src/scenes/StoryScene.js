...
export class StoryScene extends Phaser.Scene {
  ...
  create(data = {}) {
    // Acepta la selección tanto si llega como gameData como si llega suelta.
    // Esto evita perder Killa/Rumi después de la historia.
    this.gameData = data.gameData || {
      level: data.level || 1,
      playerCount: data.playerCount || 1,
      characterId: data.characterId || data.selected || data.character || 'auki',
      characterName: data.characterName,
      selected: data.selected || data.character || data.characterId || 'auki',
    };
    this.page = 0;
    this.storySprites = [];
  ...
  renderPage() {
    ...
    if (chapter.visual === 'flame') {
      ...
      heroes.forEach(({ x, data }, index) => {
        const hero = add(
          this.add.image(x, 288, data.idle || 'heroe_idle')
            .setScale(data.previewScale || 0.62)
        );

        if (data.tint && data.tint !== 0xffffff) {
          hero.setTint(data.tint);
        }

        add(
          this.add.text(x, 354, data.name, {
            fontSize: '18px',
            color: '#ffcc5c',
            fontStyle: 'bold',
            stroke: '#000',
