import { SAVE_KEY, SETTINGS_KEY, LEADERBOARD_KEY } from '../config/constants.js';
...
export class SaveManager {
  ...
  static addLeaderboard(entry) {
    const board = SaveManager.getLeaderboard();
    board.push({ ...entry, date: new Date().toISOString() });
    board.sort((a, b) => (b.score || 0) - (a.score || 0));
    const next = board.slice(0, 10);
